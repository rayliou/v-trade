...
class IBKR:
    ...
    def reqMktData(self, symList):
        ...
        while wait:
            self.ib_.sleep(0.1)
            #bList = [t.marketPrice == t.marketPrice for t in tList]
    ...
    def get_M_HistoryData(self, symList,
        ...
        ax1.set_title(f'Close of {symList}')
        ax1.set_ylabel('Close Price')
        ax2  = fig.add_subplot(212)
        df['iv/hv'] = df.iv/df.hv
        df[['iv','hv' ]].plot(ax=ax2)
        df[['iv/hv' ]].plot(ax=ax2,secondary_y=True)
        df.to_csv(symList + '.csv', index=False)
        ax2.set_title(f'iv,hv,ratio of {symList}')
        ax2.set_ylabel('iv,hv,ratio')
        ...
        df[['q_iv']].plot(ax=ax1,secondary_y=True, ylabel='q_iv')
...
if __name__ == '__main__':
    ...
    df = ibkr.get_M_HistoryData(symList,durationStr='1 Y', barSizeSetting='1 day',years=2)
